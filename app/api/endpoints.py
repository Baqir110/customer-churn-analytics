...
async def predict_churn(features: CustomerFeatures):
    if not os.path.exists(MODEL_PATH):
        raise HTTPException(status_code=500, detail="Model binary not found. Run training script first.")
    ...
    if prob >= 0.70:
        risk = "CRITICAL"
        strategy = "Trigger priority outbound retention call and offer 20% renewal discount."
    elif prob >= 0.40:
        risk = "MODERATE"
        strategy = "Send proactive service check-in survey and targeted onboarding tutorial."
    else:
        risk = "LOW"
